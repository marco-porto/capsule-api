...
export default async function weatherTypes(request: NextApiRequest,response: NextApiResponse){
    ...
    if(request.method === 'GET'){
            //Fetch ipma weather types
            const ipmaWeatherTypesResponse = await fetch('https://www.ipma.pt/bin/file.data/weathertypes.json');
            const ipmaWeatherTypesResponseJson = await ipmaWeatherTypesResponse.json();

            //Define 7 days of response cache (604800 seconds)
            response.setHeader('Cache-Control','s-maxage=604800,stale-while-revalidate'); 

            //Return ipmaWeatherTypesResponseJson
            response.json(ipmaWeatherTypesResponseJson);
    }else{
